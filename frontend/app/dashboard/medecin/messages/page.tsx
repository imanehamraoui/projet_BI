    ...
    <div style={{
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'Messages', active: true, svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z" fill="#1565C0"/></svg>, onClick: () => router.push('/dashboard/medecin/messages') },
      ...
      <div style={{ marginLeft: '90px', flex: 1, padding: '24px' }}>
        <div style={{
          ...
          <h1 style={{ color: '#1565C0', marginBottom: '16px' }}>💬 Messages</h1>
          <p style={{ color: '#666' }}>Page Messages - Consultez vos messages importants</p>
          ...
          <div style={{
            ...
            {[
              { from: 'Dr. Hassan', message: 'Consultation rapide disponible?', time: '10:30' },
              { from: 'Infirmier Ali', message: 'Résultats d\'analyse disponibles', time: '09:15' },
              { from: 'Pharmacie', message: 'Ordonnance confirmée', time: '08:45' },
              { from: 'Admin', message: 'Mise à jour système ce soir', time: 'Hier' },
            ].map((msg, idx) => (
              <div key={idx} style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s ease',
                background: idx === 0 ? '#E3F2FD' : 'white'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <p style={{ fontWeight: '600', color: '#1565C0', margin: '0 0 8px 0' }}>📨 {msg.from}</p>
                  <span style={{ fontSize: '12px', color: '#999' }}>{msg.time}</span>
                </div>
                <p style={{ fontSize: '14px', color: '#666', margin: 0 }}>{msg.message}</p>
              </div>
            ))}
          </div>
        </div>
